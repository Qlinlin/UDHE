...
def show_on_epoch_end2(epoch_time, valid_time, valid_loss, valid_psnr, valid_uranker, valid_musiq, valid_combined):
    """Formats validation stats including PSNR, Uranker, MUSIQ and combined score."""
    clear_line()
    print(
        f'Train time: {epoch_time} | Valid time: {valid_time} | '
        f'Valid loss: {valid_loss:>1.5f} | '
        f'PSNR: {valid_psnr:.2f} dB | '
        f'Uranker: {valid_uranker:.4f} | '
        f'MUSIQ: {valid_musiq:.4f} | '
        f'Combined Score: {valid_combined:.4f}'
    )
...
def validation_PSNR(UHD_Net_reuse, val_data_loader, device, category, save_tag=True):
    psnr_list = []
    ssim_list = []
    image_psnr_pairs = []  # List to store (PSNR, image_name) pairs
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        psnr_values = to_psnr(enhance, gt)
        psnr_list.extend(psnr_values)
        image_psnr_pairs.extend(zip(psnr_values, image_name))
    ...
    # Sort the list of (PSNR, image_name) pairs in descending order of PSNR
    image_psnr_pairs.sort(reverse=True, key=lambda x: x[0])

    # Print the top 10 PSNR values and corresponding image names
    print("Top 10 PSNR values with corresponding image names:")
    for i in range(min(10, len(image_psnr_pairs))):
        print(f"Image: {image_psnr_pairs[i][1]}, PSNR: {image_psnr_pairs[i][0]}")
